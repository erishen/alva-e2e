...
## 应聘提交说明（Alva · AI-Native QA 笔试 · Part 2）

> 本项目就是针对 Alva 公开 Playbook 页面的**生产环境金融数据巡检**套件，对应 JD 里「生产环境质量巡检……金融数据正确性（比如 P/E 算错了要能被巡检抓到）」这条核心职责。下面三节按笔试要求说明。

### 1. 为什么选「Playbook 数据正确性」这个场景（而不是另一个）

笔试 Part 1 的旅程是「注册 → 建 Portfolio Watch Automation → 建 Playbook → 收 Alert」，全部在登录态下。我刻意**没有**把自动化测试放在这条登录链路，而是放在**已发布的公开 Playbook（AMD Deep-Dive）的数据正确性**上，理由：
...
- 登录链路（Part 1 旅程）我没有丢弃，而是作为**互补的后续工作**——见仓库根 `PART1-onboarding.md` 的策略与脚手架（需登录态，故未纳入本自动化套件）。
...
- **登录态功能完全没覆盖。** 注册、Automation 创建、Alert 配置与推送——这些 Part 1 旅程的核心，因需真实账号而被本套件排除。它们的正确性目前**没有任何自动化保障**（见 `PART1-onboarding.md`）。
...
- 全量运行日志见 [`EVIDENCE.md`](./EVIDENCE.md)：**96 passed + 1 failed**（那 1 个 failed 即上文抓到的 `$0.0` 真实数据缺陷），约 2.5 分钟跑完。
