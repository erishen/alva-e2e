...
| 活跃功能缺陷（P0/P1） | **1 条确认**：D-1 可比公司表 EV 与 Market cap 全部渲染为 `$0.0`（疑似未接入数据源 / 字段未实现，P1；无论根因为何客观为错误数据） |
...
### D-1 【确认缺陷 · 高/P1】可比公司表 EV 与 Market cap 全部为 `$0.0`（疑似未接入数据源 / 字段未实现）
- **模块**：可比公司表（Comparables）/ 估值字段
- **现象（真 DOM 抓取验证）**：`/u/lake/playbooks/amd-deep-dive` 的可比公司表中，**每一家**公司的 `EV` 与 `Market cap` 两列均渲染为 `$0.0`（含 Average / Median / All-peer 聚合行，整列 11 行一致）。结合整列整齐缺失、聚合行亦为空，更像是这两列在 Comps 视图下**未接入数据源（或字段预留但前端未渲染）**，而非随机取数失败；但无论根因为何，AMD 为千亿美元级公司，显示 `$0.0` **客观上就是错误数据**。
...
- **可视化证据（已随仓库提交，一眼自解释）**：守卫除断言外额外生成两张截图并 `attach` 到 HTML 报告（`pnpm run report` 直接看），均由本机 `pnpm test` 真机产出，已提交至 `evidence/`，评审人无需重跑即见真实页面状态。实现要点（曾踩坑截出白屏，已规避）：用 `beforeAll` 真正加载好的 `page` 经 `dashboardFrame()` 取跨域 iframe，先点 **Comps Tab** 让可比表可见，再在 iframe 内截；不用 test fixture 空白 `page`、不用 `page.screenshot()`（跨域 OOPIF 留白）。
  ...
  **主证据**（红框高亮 EV / Market cap 两列，全部 `$0.0`）：

  ![D-1 主证据：可比表 EV / Market cap 两列全部 $0.0](../evidence/comps-ev-mc-zero.png)

  **上下文证据**（整段 Comps 区块 + 红色标注横幅 + AMD Deep-Dive 真实页面）：

  ![D-1 上下文证据：Comps 区块整段](../evidence/comps-full.png)
